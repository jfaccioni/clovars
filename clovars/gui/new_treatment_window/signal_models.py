...
MODELS = {
    'Sinusoidal': [INITIAL_VALUE, PERIOD],
    'Stochastic': [INITIAL_VALUE, NOISE],
    'Stoch. + Sin.': [INITIAL_VALUE, PERIOD, NOISE],
    'Gaussian': [INITIAL_VALUE, MEAN, STD],
    'E. M. Gaussian': [INITIAL_VALUE, MEAN, STD, K],
...
class CellSignalModel(qtc.QAbstractTableModel):
    ...
    def __init__(
        ...
        self._data = pd.DataFrame({
            'use': False,
            'name': [param.name for param in self.params],
            'value': [param.value for param in self.params],
        })
        self._limits = [{
