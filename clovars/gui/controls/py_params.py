...
class PyParams(qtc.QObject):
    ...
    @qtc.Property(bool, notify=activeChanged)
    ...
    def active(
        ...
        if new_active_state != self._active:
            ...
            self.activeChanged.emit(new_active_state)


def mainloop(qml_path: str) -> None:
    """Creates a simple ParamModel attached to a ParamViewWindow and displays it."""
    # app = qtw.QApplication(sys.argv)
    # view = qtqw.QQuickWidget(source=qml_path)
    # view.setResizeMode(qtqw.QQuickWidget.SizeRootObjectToView)
    # engine, root = view.children()
    # engine.rootContext().setContextProperty('_pyParam', ParamModel())
