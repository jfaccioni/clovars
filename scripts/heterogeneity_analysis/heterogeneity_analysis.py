...
def main(
    ...
    for colony_name, colony_df in df.groupby('colony_name'):
        values = colony_df.groupby('branch_info', as_index=False)['simulation_frames'].max()['branch_info']
        results = dict()
        ...
        _out_df = to_frame(results=filled_results, colony_name=colony_name)
        out_dfs.append(_out_df)
    out_df = pd.concat(out_dfs, ignore_index=True)
    output_folder = input_folder / 'het_analysis'
    output_folder.mkdir(exist_ok=True, parents=True)
    output_path = output_folder / f'{input_file.stem}_het_analysis{input_file.suffix}'
