...
class TestTreatmentDrawer(unittest.TestCase):
    ...
    def test_show_gaussians_method_calls_plt_show(self) -> None:
        """Tests whether the "show_gaussians" method calls "plt.show" (i.e. it displays a plot)."""
        with mock.patch('clovars.simulation.view.treatment_drawer.plt.show', side_effect=plt.close) as mock_plt_show:
            self.treatment_drawer.show_gaussians()
        mock_plt_show.assert_called_once()
        plt.show()

    def test_render_gaussians_method_saves_figure(self) -> None:
        """Tests whether the "render_gaussians" method calls "plt.Figure.savefig" (i.e. it saves a figure)."""
        with mock.patch('clovars.simulation.view.treatment_drawer.plt.Figure.savefig') as mock_savefig:
            self.treatment_drawer.render_gaussians(
    ...
    def test_yield_gaussians_method_yields_tuple_of_figure_and_label(self) -> None:
        """Tests whether the "yield_gaussians" method yields a tuple containing a matplotlib Figure and a string."""
        return_value = list(self.treatment_drawer.yield_gaussians())
    ...
    def test_yield_gaussians_yields_one_tuple_per_treatment(self) -> None:
        """Tests whether the "yield_gaussians" method yields one tuple for each treatment."""
    ...
    def test_yield_gaussians_yields_empty_sequence_if_no_treatments_are_present(self) -> None:
        """Tests whether the "yield_gaussians" method yields nothing if no treatments are present."""
        self.treatment_drawer.treatment_data = {}
        return_value = list(self.treatment_drawer.yield_gaussians())
        self.assertEqual(len(return_value), 0)

    def test_yield_gaussians_calls_death_curve_plot_pdf_if_death_gaussian_is_true(self) -> None:
        """Tests whether the "yield_gaussians" method draws the death Curve if the "show_death" parameter is True."""
        return_value = list(self.treatment_drawer.yield_gaussians(show_death=False))
        with self.assertSequenceNotEmpty(return_value):
            for _, label in return_value:
                self.assertNotIn('death', label)
                plt.close()  # Do not keep the figure open
        return_value = list(self.treatment_drawer.yield_gaussians(show_death=True))
        with self.assertSequenceNotEmpty(return_value):
            for _, label in return_value:
                self.assertIn('death', label)
                plt.close()  # Do not keep the figure open

    def test_yield_gaussians_calls_division_curve_plot_pdf_if_division_gaussian_is_true(self) -> None:
        """
        Tests whether the "yield_gaussians" method draws the division Curve
        if the "show_division" parameter is True.
        """
        return_value = list(self.treatment_drawer.yield_gaussians(show_division=False))
        with self.assertSequenceNotEmpty(return_value):
            for _, label in return_value:
                self.assertNotIn('div', label)
                plt.close()  # Do not keep the figure open
        return_value = list(self.treatment_drawer.yield_gaussians(show_division=True))
        with self.assertSequenceNotEmpty(return_value):
            for _, label in return_value:
